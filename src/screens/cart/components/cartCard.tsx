...
const CartCard = ({item}: {item: CartData}) => {
  ...
  const RemoveFromCart = async () => {
    try {
      const response = await dispatch(RemoveCart(item?.cart_items[0]?.product_cart_id)).unwrap();
  ...
  const incrementQty = async () => {
    try {
      const response = await dispatch(updateCart({quantity: +1, id: item?.cart_items[0]?.product_cart_id}));
