...
import {OfferProduct} from '../../redux/actions/homeAction';
...
const Products: React.FC = ({route}: any) => {
  ...
  const {offerData, offerLoading, total, nextPage, prevPage} = useAppSelector(
    state => state.home,
  );
  ...
  const handleWishlist = async (productId: number) => {
    try {
      ...
      dispatch(
        OfferProduct({item, page_size: ITEMS_PER_PAGE, page: currentPage}),
      );
    } catch (error) {
